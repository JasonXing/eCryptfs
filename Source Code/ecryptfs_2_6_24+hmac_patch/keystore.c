...
/***************************************HMAC PATCH BEGIN********************************************/
int
ecryptfs_write_tag_17_packet(char *dest, size_t remaining_bytes, size_t offset,
			     struct ecryptfs_crypt_stat *crypt_stat,
			     size_t *packet_size)
{

	/*
	 *              ***** TAG 17 Packet Format *****
	 *    | Content Type                       | 1 byte       |
	 *    | Hash Identifier Length               | 1 or 2 bytes |
	 *    | Hash Identifier                    | arbitrary    |
	 *    | Root Hmac Length                   | 1 or 2 bytes |
	 *    | Root Hmac                          | arbitrary    |
	 */
	...
	max_packet_size = 1 			/* Content Type*/
			+ 2			/* Hash Identifier Length*/
			+ ECRYPTFS_MAX_HMAC_NAME_SIZE	/* Hash Identifier */
			+ 2			/* Root Hmac Length */
			+ 128;			/* Root Hmac */
	if (max_packet_size > remaining_bytes) {
		ecryptfs_printk(KERN_ERR, "Insufficient space remaining in "
				"page to write tag 17 packet");
		rc = -EIO;
		goto out;
	}
	*packet_size = offset;
	dest[offset++] = ECRYPTFS_TAG_17_PACKET_TYPE;/* Content Type*/
	hmac_code = ecryptfs_code_for_hmac_string(crypt_stat);
	dest[offset++] = hmac_code;/* Hash Identifier Length*/
	rc = write_packet_length(&dest[offset], 2 * crypt_stat->hmac_bytes,
				 &packet_size_len);/* Root Hmac Length */
	if (rc) {
		ecryptfs_printk(KERN_ERR, "Error generating tag 17 packet "
				"header, cannot generate packet length\n");
		goto out;
	}
	offset += packet_size_len;
	crypt_stat->root_hmac_header_offset = offset;
	offset += (2 * crypt_stat->hmac_bytes);/* It might be 64 * 2 Octets */
				 ...
				 size_t *packet_size, size_t max_packet_size)
{
	 ...
	 *              ***** TAG 17 Packet Format *****
	 *    | Content Type                       | 1 byte       |
	 *    | Hash Identifier Size               | 1 or 2 bytes |
	 *    | Hash Identifier                    | arbitrary    |
	 *    | Root Hmac Length                   | 1 or 2 bytes |
	 *    | Root Hmac                          | arbitrary    |
	 */
	...
	start_offset = offset;
	if ( (offset + 1) - start_offset > max_packet_size) {
		ecryptfs_printk(KERN_ERR, "Error the packet size exceeds "
				"the max packet size\n");
		rc = -EIO;
		goto out;
	}
	if ( !(data[offset++] == ECRYPTFS_TAG_17_PACKET_TYPE ) ){/* Content Type */
		ecryptfs_printk(KERN_ERR, "Error unexpected packet type in "
				"header\n");
		rc = -EINVAL;
		goto out;
	}
	if ( (offset + 2) - start_offset > max_packet_size) {
		ecryptfs_printk(KERN_ERR, "Error the packet size exceeds "
				"the max packet size\n");
		rc = -EIO;
		goto out;
	...
	ecryptfs_hmac_code_to_string(crypt_stat->hmac, (u16)data[offset++]);
	if ( (offset + 2) - start_offset > max_packet_size) {
		ecryptfs_printk(KERN_ERR, "Error the packet size exceeds "
				"the max packet size\n");
		rc = -EIO;
	...
	rc = parse_packet_length(&data[offset], &size/*dst*/, &packet_size_len);/* Root Hmac Length */
	if (rc) {
		ecryptfs_printk(KERN_ERR, "Error parsing tag 17 packet "
				"header, cannot read packet length\n");
		goto out;
	}
	offset += packet_size_len;
	if ( (offset + size) - start_offset > max_packet_size) {
		ecryptfs_printk(KERN_ERR, "Error the packet size exceeds "
				"the max packet size\n");
		rc = -EIO;
		goto out;
	}
	crypt_stat->hmac_bytes = size / 2;
	crypt_stat->root_hmac_header_offset = offset;
	*packet_size = offset - start_offset;
out:
	return rc;
}
...
int ecryptfs_parse_packet_set(struct ecryptfs_crypt_stat *crypt_stat,
			      unsigned char *src,
//HMAC PATCH     struct dentry *ecryptfs_dentry)
			      struct dentry *ecryptfs_dentry,	/* HMAC PATCH */
			      size_t *bytes_read)		/* HMAC PATCH */
{
	size_t i = 0;
