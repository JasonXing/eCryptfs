 ...
 * Returns zero on success; non-zero otherwise
 */
 /* Send message to userspace daemon */ 
int ecryptfs_send_miscdev(char *data/*src*/, size_t data_size/*src*/,
			  struct ecryptfs_msg_ctx *msg_ctx/*dst*/, u8 msg_type/*src*/,
			  u16 msg_flags/*src*/, struct ecryptfs_daemon *daemon/*dst*/)
{
	int rc = 0;
	...
	mutex_lock(&daemon->mux);
	list_add_tail(&msg_ctx->daemon_out_list/*new*/, &daemon->msg_ctx_out_queue/*head*/);
		       ...
		       size_t count, loff_t *ppos)
{
	...
	if (copy_from_user(packet_size_peek/*to*/, &buf[PKT_LEN_OFFSET]/*from*/,
			   sizeof(packet_size_peek))/* Copy a block of data from user space. */) {
		printk(KERN_WARNING "%s: Error while inspecting packet size\n",
		       __func__);
		return -EFAULT;
	}

	rc = ecryptfs_parse_packet_length(packet_size_peek/*src*/, &packet_size/*dst*/,
					  &packet_size_length);
	if (rc) {
...
int __init ecryptfs_init_ecryptfs_miscdev(void)
{
	...
	rc = misc_register(&ecryptfs_miscdev);/* Register a miscellaneous device */
	if (rc)
...
void ecryptfs_destroy_ecryptfs_miscdev(void)
{
	...
	misc_deregister(&ecryptfs_miscdev);/* Unregister a miscellaneous device */
}
